...
const Ingredient = (props: IngredientProps) => {
  ...
  console.log(ingredient.primaryName, height);
  const trimString = (string: string) =>
    string.length > 23 ? string.slice(0, 23) + "..." : string;

  return (
    <div key={ingredient.ingredientId} className="relative">
      ...
      <div
        className="invisible absolute z-10 w-44 rounded-md rounded-tl-none bg-orange-200 p-4 text-xs opacity-0 shadow-xl shadow-orange-800/20
          before:absolute before:-left-2 before:top-0 before:border-b-8 before:border-r-8 before:border-t-0 before:border-transparent before:border-r-orange-200 
      ...
      >
        <div className="flex flex-col gap-2 text-center text-stone-600">
          <div>{trimString(ingredient.altNames)}</div>
